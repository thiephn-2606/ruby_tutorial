...
class CheckTimeOt
  ...
  def check_time_OT
    ...
    if @time_checkin >= @time_checkout
      ...
      if time_OT > 3 &&  @time_checkout > Time.strptime("21:00", '%H:%M').to_time
        puts "OT: #{time_OT}, Lunch: N, Dinner: Y"
      elsif time_OT > 4 && (@time_checkin.hour..@time_checkout.hour).to_a.include?(12) &&
        (@time_checkin.hour..@time_checkout.hour).to_a.include?(13)
